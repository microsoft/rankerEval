...
class Metric(object):
    ...
    def mean(self, y_true, y_pred, nan_handling='propagate',
...
class BinaryMetric(Metric):
    ...
    def score(self, y_true, y_pred):
        if not isinstance(y_true, BinaryLabels):
            raise TypeError(f"y_true must be of type BinaryLabels but is of instance {type(y_true)}")
...
class FirstRelevantRank(BinaryMetric):
    """
            Used for evaluating permutations of `y_true`. Does not accept *k* as it
            scores permutations.
    """
...
class AP(BinaryMetric):
    ...
    def _score(self, y_true, y_pred_labels):
        ...
        scores[n_pos > 0] = np.sum(
            precision * labels, axis=-1)[n_pos > 0] / np.clip(n_pos[n_pos > 0], None, self._k)
        scores[n_pos == 0] = np.NaN
